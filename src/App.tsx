...
type Screen = 'landing' | 'map' | 'mission' | 'decision' | 'result' | 'minigame' | 'practice' | 'achievements' | 'final'
...
export default function App() {
  ...
  function restart() { setSave({ ...freshSave(), practiceBest: save.practiceBest, sound: save.sound, reduceMotion: save.reduceMotion }); setSelected('school'); setModal(null); navigate('landing') }
  ...
  return <div className={`app app--${screen} ${save.reduceMotion ? 'reduce-motion' : ''}`}><div ref={mainRef} inert={modal !== null}>
    ...
    {screen === 'practice' && <main className="screen recycling-screen practice-screen" data-screen="practice">{header}<WoodTitle>Práctica de <em>separación</em></WoodTitle><p className="recycling-level">Modo infinito · 32 objetos · Cada acierto acelera el reto</p><div className="recycling-world practice-world"><div className="recycle-scenery" aria-hidden="true"><WorldArt zone="park" improved choice="restore" characters={false}/></div><div className="recycle-friends" aria-hidden="true"><svg viewBox="0 0 340 240"><Friends/></svg></div><Practice best={save.practiceBest} reduceMotion={save.reduceMotion} suspended={modal !== null} onBest={practiceBest => setSave(current => practiceBest <= current.practiceBest ? current : { ...current, practiceBest })} onSound={chime} onExit={() => navigate('map')}/></div></main>}
